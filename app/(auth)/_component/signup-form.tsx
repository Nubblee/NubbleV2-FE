...
const SignUpForm = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (isNickFocused && nicknameRef.current && !nicknameRef.current.contains(e.target as Node)) {
        handleAvailableNickName({ nickname })
        setIsNickFocused(false)
      }

      if (isIdFocused && loginIdRef.current && !loginIdRef.current.contains(e.target as Node)) {
