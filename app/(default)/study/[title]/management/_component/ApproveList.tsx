...
const ApproveList = () => {
  ...
  const handleCheckedList = (id: number, isChecked: boolean) => {
    ...
    <div className="flex flex-col w-full gap-4">
      ...
      {approveItems.map((item) => (
        <ApproveCard
          key={item.requestId}
          {...item}
          checked={!!checkedMap[item.requestId]}
          onChange={(checked) => handleCheckedList(item.requestId, checked)}
        />
      ))}
    </div>
  );
