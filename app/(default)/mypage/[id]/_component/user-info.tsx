...
const UserInfo = () => {
  ...
  return (
    <Card type='green' className='relative w-full p-12 flex items-center gap-10'>
      <Link
        href={`/mypage/${user?.loginId}/profile`}
      ...
      <ProfileImage
        src={user?.profileImg ?? '/basicProfile.png'}
      ...
      <section aria-label='사용자 정보'>
        <h2 className='text-xl font-bold flex items-baseline gap-1'>
          <span className='text-2xl mb-4'>{user?.nickname}</span>
          <span className='text-lg font-medium'>님</span>
        </h2>
        <p className='text-gray-middle'>{user?.email}</p>
      </section>
    </Card>
  )
}
